...
<div class="page-content">
    ...
    <div class="card">
        <form id="myForm" method="POST" action="{{ route('update.supplier') }}" enctype="multipart/form-data">
            @csrf
                    ...
                    <div class="col-lg-8">
                        <div class="border border-3 p-4 rounded">
                            <div class="mb-3">
                                ...
                                <input type="hidden" name="id" value="{{ $sup->id}}" readonly>
                                <input type="text" value="{{ $sup->nama }}" name="nama_supplier" class="form-control" id="nama_supplier" placeholder="Ketik Nama Supplier">
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Jenis Kelamin</label>
                                <select name="jenis_kelamin" class="form-control">
                                    <option value="">-- Jenis Kelamin --</option>
                                    <option value="L" {{ 'L' == $sup->jenis_kelamin ? 'selected' : '' }}>Laki-laki</option>
                                    <option value="P" {{ 'P' == $sup->jenis_kelamin ? 'selected' : '' }}>Perempuan</option>
                                </select>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Telpon</label>
                                <input type="text" value="{{ $sup->phone }}" name="phone" class="form-control" id="phone" placeholder="Ketik Nomor Telpon">
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Alamat</label>
                                <textarea name="alamat" class="form-control" id="alamat" rows="3" placeholder="Ketik Alamat">{{ $sup->alamat }}</textarea>
                            </div>
                            <div class="mb-3">
